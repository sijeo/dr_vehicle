...
typedef struct {
    ...
    // GNSS usage diagnostics (for logging and debugging )
    int gnss_used_pos;      // 1 if GNSS position update accepted
    int gnss_used_vel;      // 1 if GNSS velocity update accepted
    int reacq_active;       // 1 when in reacquisition mode
    int reacq_left;         // remaining reacq fixes
    int snap_applied;       // 1 if we snapped after long outage.
...
static void* fusion_thread(void *arg) {
    ...
    while (__atomic_load_n(&C->running, __ATOMIC_ACQUIRE)) {
        ...
        C->gnss_used_vel = 0;
        C->snap_applied = 0;
        C->reacq_active = (C->reacq_left > 0) ?  1 : 0;
        ...
        if (C->have_gnss && C->gnss_have_fix) {
            ...
            /**
             * If we've been in outage for a while, enter reacquistion mode so we can relock
             * even if the state has drifted and strict gating would reject everything
             */
            if(!C->gnss_valid && C->outage_s > REACQ_START_S && C->reacq_left <= 0){
            ...
            if(!C->gnss_valid && C->outage_s > SNAP_OUTAGE_S && hdop <= SNAP_HDOP_MAX && jump_m <= SNAP_MAX_JUMP_M){
                ...
                const float v_snap = 1.0f;
                for( i=0; i<3; i++ ){
                    C->ins.P[(i+0)*15 + (0+i)] = p_snap*p_snap;
                    C->ins.P[(i+3)*15 + (3+i)] = v_snap*v_snap;
                }
                C->snap_applied = 1;
                C->gnss_valid = true;
                C->gnss_just_returned = false;
                ...
                fprintf(stderr, "GNSS SNAP applied: jump=%.1fm hdop=%.2f\n",jump_m, hdop);
            }

            // Reacq: inflate measurement noise and loosen gate for a few fixes.
            float gate_thr = CHI2_3DOF_GATE;
            if(C->reacq_active ) {
                Rpos *= REACQ_R_MULT;
                gate_thr *= REACQ_GATE_MULT;
            }
